...
InstitutionModel.save = async (data, condition = []) => {
...
InstitutionModel.delete = async (condition) => {
...
InstitutionModel.getBy = async ({condition = [], join = [], group = [], fields = []}) => {
...
InstitutionModel.getAll = async ({condition = [], join = [], group = [], sort = [], page = null, limit = null, fields = ['*']}) => {
    const now = new Date();
    CoreDB.select(tableName);
    CoreDB.setFields(fields);
    if (condition.length > 0) {
...
InstitutionModel.getCount = async ({condition = [], join = [], group = [], sort = []}) => {
    CoreDB.select(tableName);
    CoreDB.setFields([`COUNT(${tableName}.Institution_id) AS total`]);
...
InstitutionModel.saveBatch = async ({field = [], data = []}) => {
    CoreDB.insert(tableName);
